...
export default async function handler(req, res) {
  ...
  if (req.method === 'DELETE') {
    ...
    if (error) return res.status(500).json({ error: String(error.message || error) })
    // Remove the stored PDF by deterministic prefix — regardless of the pdf_path
    // column, so files orphaned by a failed path update stay reachable (orphan
    // rule, storage-blueprint.md). Best-effort: a storage error never fails the delete.
    await removeByPrefix(supabase, 'cvs', id + '.')
